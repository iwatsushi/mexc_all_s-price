...
class MEXCWebSocketProcess:
    ...
    def __init__(self, config: Config, data_queue: multiprocessing.Queue, control_queue: multiprocessing.Queue):
        """
        初期化
    ...
    async def _connect_and_run(self):
        ...
        try:
            async with websockets.connect(ws_url) as websocket:
                ...
                logger.info(f"💓 MEXC ping initialized ({self._ping_interval}s interval)")
...
def mexc_websocket_worker(config_dict: dict, data_queue: multiprocessing.Queue, control_queue: multiprocessing.Queue):
