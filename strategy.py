...
class TradingStrategy:
    ...
    def analyze_tick(self, tick: TickData) -> TradingSignal:
        ...
        with self._lock:
            ...
            if tick.symbol in self.position_trackers:
                ...
                # 決済シグナルをチェック
                close_signal = self._check_close_signal(tick.symbol, tick.price, tick.timestamp)
    ...
    def _check_close_signal(self, symbol: str, current_price: float, timestamp: datetime) -> TradingSignal:
        """決済シグナルをチェック"""
        ...
        if tracker.side == "LONG":
            profit_percent = ((current_price - tracker.entry_price) / tracker.entry_price) * 100.0
        else:  # SHORT
            profit_percent = ((tracker.entry_price - current_price) / tracker.entry_price) * 100.0
        ...
        if profit_percent > tracker.max_profit_percent:
            ...
            # 最小利益閾値を超えた場合
            if not tracker.min_profit_threshold_reached and profit_percent >= self.min_profit_percent:
                tracker.min_profit_threshold_reached = True
                tracker.breakeven_stop_set = True
                logger.info(f"{symbol}: Min profit threshold reached. Breakeven stop set.")
    ...
    def _check_reversal_exit(self, tracker: PositionTracker, current_price: float) -> Optional[str]:
        """反発による決済をチェック"""
        ...
        if tracker.side == "LONG":
            ...
            if tracker.highest_price_after_entry > 0:
                reversal_percent = ((tracker.highest_price_after_entry - current_price) 
                                 / tracker.highest_price_after_entry) * 100.0
        else:
            # ショートポジション：最安値からの上昇率
            if tracker.lowest_price_after_entry < float('inf'):
                reversal_percent = ((current_price - tracker.lowest_price_after_entry) 
                                 / tracker.lowest_price_after_entry) * 100.0
