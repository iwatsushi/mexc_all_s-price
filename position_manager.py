...
class PositionManager:
    ...
    def _update_account_info(self):
        ...
        try:
            ...
            if balance_response.get("success"):
                ...
                logger.error(f"Failed to get balance: {balance_response.get('message')}")
    ...
    def _calculate_position_size(self, symbol: str, side: str, entry_price: float) -> Tuple[float, float, float]:
        """
        ポジションサイズを計算
        ...
        if position_value < min_position_value:
            logger.warning(f"Position value {position_value} below minimum {min_position_value}")
            return 0.0, 0.0, 0.0
    ...
    def can_open_position(self, symbol: str) -> Tuple[bool, str]:
        ...
        with self._lock:
            ...
            if usable_capital < self.min_order_size:
                return False, f"Insufficient balance. Available: {usable_capital:.2f} USDT"
    ...
    def open_position(
        ...
        with self._lock:
            ...
            # 注文実行
            order_result = self.mexc_client.place_market_order(symbol, side, position_size)
            ...
            if order_result.success:
                ...
                logger.info(f"Position opened: {symbol} {side} size={position_size:.6f}")
                return True, f"Position opened successfully", position
                
            else:
                self.stats["failed_orders"] += 1
                logger.error(f"Failed to open position for {symbol}: {order_result.message}")
                return False, f"Order failed: {order_result.message}", None
    
    def close_position(self, symbol: str, reason: str = "") -> Tuple[bool, str, Optional[Position]]:
        """
        ポジションを閉じる
        ...
        with self._lock:
            ...
            if order_result.success:
                ...
                logger.info(f"Position closed: {symbol} {position.side} reason='{reason}'")
                return True, "Position closed successfully", closed_position
                
            else:
                position.status = PositionStatus.ERROR
                self.stats["failed_orders"] += 1
                logger.error(f"Failed to close position for {symbol}: {order_result.message}")
        ...
        with self._lock:
            return {
                **self.stats,
                ...
                "total_margin_used": self.total_margin_used,
                "account_balance": self.account_balance,
                "portfolio_value": self.account_balance + self.stats["total_unrealized_pnl"],
            }
